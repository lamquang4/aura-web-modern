...
function ProfileMenu({ profileMenuOpen }: Props) {
  ...
  return (
    ...
    >
      {account ? (
        <>
          <p className="border-b p-2.5 border-border max-w-[210px] overflow-hidden text-ellipsis whitespace-nowrap text-center">
            Xin chào, {account.fullname}
          </p>

          <Link
            className="hover:bg-bg w-full block p-2.5"
            to="/account/profile"
          >
            Thông tin tài khoản
          </Link>

          <Link className="hover:bg-bg w-full block p-2.5" to="/saved">
            Thiệp của tôi
          </Link>

          <Button
            onClick={logout}
            className="hover:bg-bg w-full block p-2.5 text-left"
          >
            Đăng xuất
          </Button>
        </>
